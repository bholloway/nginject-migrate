...
module.exports = function findAnnotatedRanges(ast) {
  var pending   = ast.comments
    .filter(testDocTag)
    .map(getAnnotatedNode),
      additions = pending
        .filter(noCorrespondingError(pending))
        .filter(truthyFirstOccurance) // ensure unique values
        .reduce(itemToAddOperation, []),
      removals  = ast.comments
        .filter(noCorrespondingError(pending))
        .reduce(itemToRemoveOperation, []),
      errors    = pending
        .filter(isError);
  return {
    additions: additions,
 ...
 */
function itemToAddOperation(list, node) {
 ...
 * @returns {{start:{line:number,column:number},end:{line:number,column:number}} The location of the insertion
 */
function itemToRemoveOperation(list, node) {
  var PURE_COMMENT = /^[\W\\n]*@ngInject[\W\\n]*$/;
