...
function applyAlterations(sourcePath, content, pending, quoteChar) {
   ...
   * @param {{line:number, column:number}} start A zero-based start location from where we might insert
   * @param {{line:number, column:number}} end A zero-based end location to where we might insert
   */
  function addition(start, end) {
    ...
    if (isInline) {
      ...
      generator.addMapping({
        ...
        generated: oneBased({
          line  : startZero.line,
          column: startZero.column + directive.length
        })
      });
    }
    // comment on a new line
    else {
      var iLineFirst = startZero.line + 1,
          iLineLast  = endZero.line,
          text       = lines[iLineFirst],
          indent     = lines.slice(iLineFirst, iLineLast).reduce(reduceIndent, text),
          eol        = text.match(/\r?\n/)[0],
          insertion  = indent + directive + eol;

      lines.splice(iLineFirst, 0, insertion);
      generator.addMapping({
        source   : sourcePath,
        original : start,
        generated: oneBased({
          line  : iLineFirst,
          column: startZero.column + insertion.length
        })
      });
    }
  }
   ...
   * @param {object} loc A location range where we might insert
   */
  function removal(loc) {
// TODO
  }
}
