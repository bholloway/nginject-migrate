...
function applyAlterations(content, pending, options) {
  ...
  function addition(start, end) {
    ...
    // comment on a new line
      ...
      // source-map change
      //  the start of the next segment is one character more than what we have added
      mapper.addMapping({
        original : {
          line  : firstLine,
          column: 0
  ...
  function removal(start, end) {
    ...
    var isWholeLine = (lines[end.line].length === 0);
    mapper.addMapping({
      original : isWholeLine ? {line: end.line + 1, column: 0} : end,
      generated: start
    });

    function eachLine(text, i, array) {
      var index = start.line + i;

      // first and last line in one
      if (array.length === 1) {
        lines[index] = text.slice(0, start.column) + text.slice(end.column);
      }
      // multiple lines
      else {
        var isFirst = (i === 0),
            isLast  = (i === array.length - 1);
        lines[index] = isFirst ? text.slice(0, start.column) : isLast ? text.slice(end.column) : '';
      }
    }
  }
}
