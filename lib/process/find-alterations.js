...
function itemToRemoveRange(list, commentNode) {
  ...
  // otherwise we just remove the lines that are pure
    ...
    return commentNode.value.split(/(\r?\n)/)
  ...
  function eachLine(list, text, i, array) {
    if ((i % 2 === 0) && PURE_COMMENT.test(text)) {
      var line = commentNode.loc.start.line + i / 2;
      return list.concat({
        start: {
          line  : line,
          column: (i === 0) ? commentNode.loc.start.column : 1
        },
        end  : {
          line  : line,
          column: (i === array.length - 1) ? commentNode.loc.end.column : (text.length + array[i + 1].length)
        }
      });
    } else {
      return list;
