...
function sourceMapHelper(filename, sourceMap) {
  ...
  function offsetMapping(mapping) {
    ...
    function eachMapping(existing) {
      var isUpdate = (dLine !== 0) ? (existing.generatedLine > mapping.original.line) :
        ((existing.generatedLine === mapping.original.line) && (existing.generatedColumn >= mapping.original.column));
      if (isUpdate) {
