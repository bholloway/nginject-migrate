...
var esprimaTools        = require('./esprima-tools'),
    ...
    getErrorAccumulator = require('./get-error-accumulator');
...
module.exports = function process(content, sourceMapOrFilename) {
  ...
  // process the content
    ...
    // make insertions and deletions
      ...
      // don't remove anything for which there was an error any copy errors to the result
      //  reduce right-to-left to ensure that iterator is not effected by splicing
      insertionRanges.reduceRight(getErrorAccumulator(insertionRanges, removalRanges), result);

      // now make the changes
      //  source-map is mutated in-place
